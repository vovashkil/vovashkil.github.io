...
DTL permits extending a base template like this: **`{% "pets_app/base.html" %}`**. Note that the base template might be placed in the same application or in a different application. The consequence of using **`extends`** is that the block content in the parent template, defined as **`{% block content %}` ... `{% endblock content %}`**, is overridden by the corresponding block content in the child template.
